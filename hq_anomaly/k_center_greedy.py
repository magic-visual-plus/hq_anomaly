    ...
    def select_coreset_idxs(self) -> list[int]:
        ...
        if self.embedding.ndim == 2:
            if self.model is None:
                # self.model = PCA(original_dim=self.embedding.shape[1], target_dim=256)
                self.model = SparseRandomProjection(eps=0.99)
                # self.model.fit(self.embedding)
                pass
            self.model.fit(self.embedding)
            self.features = self.model.transform(self.embedding.float())
            self.features = self.features.to(dtype=self.embedding.dtype)
